...
def create_uff(data_type="real"):
    ...
    for elem_idx in range(n_elem):
        T = Translation(
            float(elem_x[elem_idx]), float(elem_y[elem_idx]), float(elem_z[elem_idx])
        )
    ...
    probe_T = Translation(
        1.0, float(Ny // 2 - transducer_width // 2), float(Nz // 2 - elem_length // 2)
    )
    ...
    transmit_waves = [
        TransmitWave(
            wave=1, time_zero_reference_point=TimeZeroReferencePoint(0.0, 0.0, 0.0)
        )
    ]
    ...
    us_event = Event(transmit_setup=transmit_setup, receive_setup=receive_setup)
    ...
    unique_waves = [
        Wave(
            origin=Origin(position=Position(), rotation=Rotation()),
            wave_type=WaveType.PLANE,
            aperture=Aperture(
                origin=Position(), fixed_size=0.0, f_number=1.0, window="rectwin"
            ),
            excitation=1,
        )
    ]
