...
PRIMITIVE_INTS = Union[int, np.int32, np.int64]
PRIMITIVE_FLOATS = Union[float, np.float32, np.float64]
PRIMITIVES = Union[np.ndarray, bytes, str, PRIMITIVE_INTS, PRIMITIVE_FLOATS]
...
def _recursively_save_dict_contents_to_group(h5file, path, dic):
    ...
    for key, item in dic.items():
        ...
        elif isinstance(item, get_args(PRIMITIVES)):
